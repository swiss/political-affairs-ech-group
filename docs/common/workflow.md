...
> **Caveat (case-insensitive filesystems):** `gen-doc` writes one file per element, named exactly after it. A class `Protocol` and a slot `protocol` would produce `Protocol.md` and `protocol.md`, which collide on macOS/Windows. Avoid element names that differ only by case (e.g. name the slot `protocol_ref`). This includes the built-in LinkML types, which `gen-doc` documents alongside your own elements: a slot named `date` collides with the type `Date`, and likewise `string`, `integer`, `uri` and friends. Such a collision only shows on a developer machine — Linux CI writes both files happily, and the checkout then flip-flops between them.
