---
search:
  boost: 5.0
---

# Slot: individual_attendances 


_Sammlung der einzelnen Anwesenheitsfeststellungen._




<div data-search-exclude markdown="1">



URI: [ops:individualAttendance](https://ch.paf.link/schema/operations/individualAttendance)
<!-- no inheritance hierarchy -->





## Anwendbare Klassen

| Name | Beschreibung | Ändert Slot |
| --- | --- | --- |
| [Container](Container.md) | Container für die Daten des öffentlichen Ratsbetriebs: Legislaturperioden, Se... |  no  |






## Eigenschaften

### Typ und Wertebereich

| Eigenschaft | Wert |
| --- | --- |
| Wertebereich | [IndividualAttendance](IndividualAttendance.md) |
| Domäne von | [Container](Container.md) |
| Slot-URI | [ops:individualAttendance](https://ch.paf.link/schema/operations/individualAttendance) |

### Kardinalität und Anforderungen

| Eigenschaft | Wert |
| --- | --- |
| Mehrwertig | Yes |












## LinkML-Quelle

<details>
```yaml
name: individual_attendances
annotations:
  description_de:
    tag: description_de
    value: 'Sammlung der einzelnen Anwesenheitsfeststellungen.

      '
  description_fr:
    tag: description_fr
    value: 'Ensemble des constatations individuelles de présence.

      '
description: 'Sammlung der einzelnen Anwesenheitsfeststellungen.

  '
from_schema: https://ch.paf.link/schema/operations
rank: 1000
slot_uri: ops:individualAttendance
domain_of:
- Container
range: IndividualAttendance
multivalued: true
inlined: true
inlined_as_list: true

```
</details></div>